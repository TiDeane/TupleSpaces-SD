...
public class SequencerMain {
	...
	// There can only be one Sequencer, so the port is fixed and the qualifier is always A
    ...
    public static void main(String[] args) throws IOException, InterruptedException {
		...
		final int port = PORT;
		final BindableService impl = new SequencerServiceImpl(DEBUG_FLAG);
		...
		try {
			...
			registerRequest = NameServerOuterClass.RegisterRequest.newBuilder().
				setName("Sequencer").setQualifier("A").
				setAddress(localAddress).build();

				debug("Sending Register request of service Sequencer with address " +
						localAddress + " and Qualifier A");
        ...
        Runtime.getRuntime().addShutdownHook(new Thread(() -> {
			try {
				...
				deleteRequest = NameServerOuterClass.DeleteRequest.newBuilder().
					setName("Sequencer").setAddress(localAddress).build();

				debug("Sending Delete request of service Sequencer with address " + localAddress);
